...
@dp.message_handler(text="📈 Накрутка 📈", state="*")
async def show_search(message: types.Message, state: FSMContext):
    ...
    if len(get_categories) >= 1:
        ...
        await message.answer("<b>Выберите социальную сеть для накрутки:</b>", reply_markup=get_kb, parse_mode='HTML')
    else:
        await message.answer("<b>🎁 Товары в данное время отсутствуют.</b>", parse_mode='HTML')


# Обработка кнопки "Профиль"
@dp.message_handler(text="👤 Кабинет 👤", state="*")
async def show_profile(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer(get_user_profile(message.from_user.id), reply_markup=open_profile_inl, parse_mode='HTML')
...
async def show_contact(message: types.Message, state: FSMContext):
    await state.finish()
    get_settings = get_settingsx()
    await message.answer(get_settings[0], disable_web_page_preview=True, parse_mode='HTML')
    ...
    if len(last_purchases) >= 1:
        ...
        for purchases in last_purchases:
            save_purchases.append(f"<b>📃 Чек:</b> <code>#{purchases[4]}</code>\n"
                                  f"▶ {purchases[9]} | {purchases[5]}шт | {purchases[6]}Сум\n"
                                  f"🕜 {purchases[13]}\n"
...
async def show_contact(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer(get_user_bal(message.from_user.id), reply_markup=bal_profile, parse_mode='HTML')
    
@dp.callback_query_handler(text_startswith="ball", state="*")
async def open_bal(call: CallbackQuery, state: FSMContext):
    await state.finish()
    await call.message.edit_text(get_user_bal(call.from_user.id), reply_markup=bal_profile, parse_mode='HTML')
...
async def open_category_for_buy_item(call: CallbackQuery, state: FSMContext):
    ...
    if len(get_all_podcategories1) > 0:
        ...
        if len(get_categories) >= 1:
            ...
            if category_id == 1:
                await call.message.edit_text("<b>Выберите социальную сеть для накрутки:</b>", reply_markup=get_kb, parse_mode='HTML')
            else:
                await call.message.edit_text("<b>Что хотите накрутить?</b>", reply_markup=get_kb, parse_mode='HTML')
        else:
        ...
        if len(get_all_podcategories1) > 0:
            ...
            if len(get_positions) >= 1:
                await call.message.edit_text("<b>📦Выберите тариф :</b>",
...
async def open_category_for_create_position(call: CallbackQuery, state: FSMContext):
    ...
    if len(get_position[5]) >= 5:
        ...
        await call.message.answer_photo(get_position[5],
                                        send_msg,
                                        reply_markup=open_item_func(position_id, remover, category_id), parse_mode='HTML')
    else:
...
Минимум 10 - максимум 100000""", reply_markup=all_back_to_main_default, parse_mode='HTML')
        else:
...
async def input_buy_count_item(message: types.Message, state: FSMContext):
    ...
    if message.text.isdigit():
        ...
        if get_count >= 10 and get_count <= 100000:
            if True:
                if int(get_user[4]) >= amount_pay:
                    async with state.proxy() as data:
                        data["here_cache_count_item"] = int(message.text)
                    await StorageUsers.here_cache_url.set()
                    await message.answer(f"""🔗 Отправьте ссылку для накрутким
➖➖➖➖➖➖➖➖➖➖➖➖➖
➖➖➖➖➖➖➖➖➖➖➖➖➖
▶ Количество : <code>{get_count}</code> шт
💰 Сумма к оплате : <code>{amount_pay}</code> Сум""", parse_mode='HTML')
                else:
...
# Принятие кол-ва товаров для покупки
@dp.message_handler(state=StorageUsers.here_cache_url)
async def input_buy_count_item(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        position_id = data["here_cache_position_id"]
        get_count = data["here_cache_count_item"]
    get_items = get_itemsx("*", position_id=position_id)
    get_position = get_positionx("*", position_id=position_id)
    up_position_id = get_position[7]
    while up_position_id != 1:
        up_position = get_categoryx("*", category_id=up_position_id)
        up_position_id = up_position[3]
        up_position_name = up_position[2]
    get_user = get_userx(user_id=message.from_user.id) 
    if len(message.text) < 500:
        ...
        if len(get_items) >= 0:
            if True:
                if int(get_user[4]) >= amount_pay:
                    await state.finish()
                    delete_msg = await message.answer("<b>.</b>", parse_mode='HTML')
                    await delete_msg.delete()
                    await message.answer(f"<b>🛒 Вы подтверждаете заказ ?</b>\n"
                                         f"➖➖➖➖➖➖➖➖➖➖➖➖➖\n"
                                         f"Вы делаете накрутку на: <code>{up_position_name}</code>\n"
                                         f"🏷 Название тарифа: <code>{get_position[2]}</code>\n"
                                         f"➖➖➖➖➖➖➖➖➖➖➖➖➖\n"
                                         f"▶️ Количество: <code>{get_count} шт</code>\n"
                                         f"💰 Сумма к оплате: <code>{amount_pay} Сум</code>",
                                         reply_markup=confirm_buy_items(position_id, get_count,
                                                                        delete_msg.message_id, url), parse_mode='HTML')
                
    else:
        await message.answer(f"<b>❌ Данные были введены неверно.</b>\n"
                             f"<b>📦 Введите ссылку</b>\n"
                             f"➖➖➖➖➖➖➖➖➖➖➖➖➖\n"
                             f"💳 Ваш баланс: <code>{get_user[4]}</code>\n"
                             f"🏷 Название товара: <code>{get_position[2]}</code>\n"
                             f"💵 Стоимость товара: <code>{get_position[3]}Сум</code>\n",
...
async def yes_buy_this_item(call: CallbackQuery, state: FSMContext):
    ...
    if True:
        if int(get_user[4]) >= amount_pay:
            ...
            await call.message.answer(f"{get_position[8]}", reply_markup=check_user_out_func(call.from_user.id), parse_mode='HTML')
            await send_all_admin(f"<b>🎁 Человек купил товары ✅</b>\n"
                                      ...
                                      f"💵 Сумма покупки: <code>{amount_pay}Сум</code>\n"
