...
def get_user_profile(user_id):
    ...
    msg = f"<b>📱 Ваш профиль:</b>\n" \
          ...
          f"🕜 Дата Регистрации: <code>{get_user[6]}</code>\n" \
          f"➖➖➖➖➖➖➖➖➖➖➖➖➖\n" \
          f"📦 Количество заказов в боте: <code>{count_items} шт</code>"
    return msg

def get_user_bal(user_id):
    get_user = get_userx(user_id=user_id)
    get_purchases = get_purchasesx("*", user_id=user_id)
    count_items = 0
    if len(get_purchases) >= 1:
        for items in get_purchases:
            count_items += int(items[5])
    msg = f"💳 Баланс: <code>{get_user[4]} сум</code>\n"
    return msg
