...
def position_open_create_ap(remover, cat):
    x = 0
    keyboard = InlineKeyboardMarkup()
    get_categories = get_all_categoriesx(upcategories=cat)
    for a in range(remover, len(get_categories)):
        if x < count_page:
            keyboard.add(InlineKeyboardButton(f"{get_categories[a][2]}",
    ...
    if len(get_categories) <= 10:
        ...
        nomer_kb = InlineKeyboardButton("🔸 1 🔸", callback_data="...")
        next_kb = InlineKeyboardButton("➡ Далее ➡", callback_data=f"create_position_nextp:{cat}:{remover + count_page}")
        keyboard.add(nomer_kb, next_kb)
    return keyboard

def position_open_create(remover, id_key):
    x = 0
    keyboard = InlineKeyboardMarkup()
    get_categories = get_all_categoriesx_creat()
    for a in range(remover, len(get_categories)):
        if x < count_page:
            keyboard.add(InlineKeyboardButton(f"{get_categories[a][2]}",
                                              callback_data=f"create_position_podk:{id_key}:{get_categories[a][1]}"))
        x += 1
    keyboard.add(InlineKeyboardButton(f"Общая", callback_data=f"create_position_podk:{id_key}:1"))
    if len(get_categories) <= 10:
        pass
    elif len(get_categories) > count_page:
        nomer_kb = InlineKeyboardButton("🔸 1 🔸", callback_data="...")
...
def position_create_next_page_ap(remover, cat):
    x = 0
    keyboard = InlineKeyboardMarkup()
    get_categories = get_all_categoriesx(upcategories=cat)
    for a in range(remover, len(get_categories)):
        ...
        x += 1
    if remover + count_page >= len(get_categories):
        prev_kb = InlineKeyboardButton("⬅ Назад ⬅", callback_data=f"create_position_prevp:{cat}:{remover - count_page}")
        nomer_kb = InlineKeyboardButton(f"🔸 {str(remover + count_page)[:-1]} 🔸", callback_data="...")
        keyboard.add(prev_kb, nomer_kb)
    else:
        next_kb = InlineKeyboardButton("➡ Далее ➡", callback_data=f"create_position_nextp:{cat}:{remover + count_page}")
...
def position_create_previous_page_ap(remover, cat):
    x = 0
    keyboard = InlineKeyboardMarkup()
    get_categories = get_all_categoriesx(upcategories=cat)
    for a in range(remover, len(get_categories)):
...
def position_open_edit_category_ap(remover, cat):
    x = 0
    keyboard = InlineKeyboardMarkup()
    get_categories = get_all_categoriesx(upcategories=cat)
    for a in range(remover, len(get_categories)):
    ...
    if len(get_categories) <= 10:
        ...
        keyboard.add(prev_kb, nomer_kb, next_kb)
    if cat != 1:
        get_categories = get_all_categoriesx(category_id=get_categories[0][3])
        keyboard.add(InlineKeyboardButton(f"⬅ В прердыдущую категорию ⬅", callback_data=f"position_edit_category:{get_categories[0][3]}"))
    return keyboard
...
def position_open_edit_ap(remover, category_id):
    ...
    for a in range(remover, len(get_positions)):
        if x < count_page:
            ...
            keyboard.add(InlineKeyboardButton(f"{get_positions[a][2]}",
...
def item_open_add_category_ap(remover, cat):
    x = 0
    keyboard = InlineKeyboardMarkup()
    get_categories = get_all_categoriesx(upcategories = cat)
    for a in range(remover, len(get_categories)):
    ...
    if len(get_categories) <= 10:
        ...
        nomer_kb = InlineKeyboardButton("🔸 1 🔸", callback_data="...")
        next_kb = InlineKeyboardButton("➡ Далее ➡",
                                       callback_data=f"add_item_category_nextp:{cat}:{remover + count_page}")
        keyboard.add(nomer_kb, next_kb)
    elif remover + count_page >= len(get_categories):
        prev_kb = InlineKeyboardButton("⬅ Назад ⬅",
                                       callback_data=f"add_item_category_prevp:{cat}:{remover - count_page}")
        nomer_kb = InlineKeyboardButton(f"🔸 {str(remover + count_page)[:-1]} 🔸", callback_data="...")
        keyboard.add(prev_kb, nomer_kb)
    else:
        next_kb = InlineKeyboardButton("➡ Далее ➡",
...
def buy_item_open_category_ap(remover, cat):
    ...
    if cat != 1:
        get_categories = get_all_categoriesx(category_id=get_categories[0][3])
        keyboard.add(InlineKeyboardButton(f"⬅ В прердыдущую категорию ⬅",
                                              callback_data=f"buy_open_category:{get_categories[0][3]}"))
    return keyboard
...
def buy_item_item_position_ap(remover, category_id):
    ...
    keyboard = InlineKeyboardMarkup()
    get_positions = get_positionsx("*", category_id=category_id)
    up_cat = get_categoriesx("upcategories", category_id = category_id)[0][0]
    for a in range(remover, len(get_positions)):
        if x < count_page:
            get_items = get_itemsx("*", position_id=get_positions[a][1])
            keyboard.add(InlineKeyboardButton(f"{get_positions[a][2]}",
                                              callback_data=f"buy_open_position:{up_cat}:{get_positions[a][1]}:{remover}:{category_id}"))
        x += 1
    if len(get_positions) <= 10:
        pass
    elif len(get_positions) > count_page and remover < 10:
        ...
        keyboard.add(prev_kb, nomer_kb, next_kb)
    keyboard.add(InlineKeyboardButton("⬅ Вернуться ↩",
                                      callback_data=f"back_buy_item_to_category:{up_cat}"))
    return keyboard
